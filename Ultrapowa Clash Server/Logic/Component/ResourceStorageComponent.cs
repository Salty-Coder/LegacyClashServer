...
namespace UCS.Logic
{
    class ResourceStorageComponent : Component
    {
        ...
        {
            int elixirIndex = 2; // The index of the elixir resource
            int currentElixir = m_vCurrentResources[elixirIndex];
            int maxElixir = m_vMaxResources[elixirIndex];

            // Ensure that the new amount doesn't exceed the maximum
            int newElixir = Math.Min(currentElixir + amount, maxElixir);

            // Update the current amount of elixir
            m_vCurrentResources[elixirIndex] = newElixir;
        }

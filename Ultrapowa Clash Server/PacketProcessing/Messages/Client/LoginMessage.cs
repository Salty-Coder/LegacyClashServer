    ...
    class LoginMessage : Message
    {
        ...
        public override void Process(Level level)
        {
            if (Convert.ToBoolean(ConfigurationManager.AppSettings["maintenanceMode"]))
            {
                ...
                loginFailedMessage.SetReason(ConfigurationManager.AppSettings["MaintenanceMessage"]);
            ...
            level = ResourcesManager.GetAlliance(m_vAccountId);
            if(level != null)
            {
            ...
            PacketManager.ProcessOutgoingPacket(new BookmarkMessage(this.Client, level));
            //PacketManager.ProcessOutgoingPacket(new BookmarkListMessage(base.Client));
            PacketManager.ProcessOutgoingPacket(new AllianceWarHistoryMessage(this.Client, alliance));
            //PacketManager.ProcessOutgoingPacket(new AllianceWarDataMessage(this.Client, alliance));
            if (alliance != null)
            ...
            {
                AllianceMailStreamEntry allianceMailStreamEntry = new AllianceMailStreamEntry();
                allianceMailStreamEntry.SetId((int)DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1)).TotalSeconds);
                allianceMailStreamEntry.SetSenderId(0L);
                allianceMailStreamEntry.SetSenderAvatarId(0L);
                allianceMailStreamEntry.SetSenderName(ConfigurationManager.AppSettings["welcomeMessageSender"]);
                allianceMailStreamEntry.SetIsNew(0);
                allianceMailStreamEntry.SetAllianceId(0L);
                allianceMailStreamEntry.SetSenderLeagueId(22);
                allianceMailStreamEntry.SetAllianceBadgeData(1728059989);
                allianceMailStreamEntry.SetAllianceName("System Bot");
                allianceMailStreamEntry.SetMessage(ConfigurationManager.AppSettings["welcomeMessage"]);
                allianceMailStreamEntry.SetSenderLevel(500);
                AvatarStreamEntryMessage avatarStreamEntryMessage = new AvatarStreamEntryMessage(level.GetClient());
                avatarStreamEntryMessage.SetAvatarStreamEntry(allianceMailStreamEntry);
                PacketManager.ProcessOutgoingPacket(avatarStreamEntryMessage);
